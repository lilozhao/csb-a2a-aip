...
release_name="mq-auth-server-wheel-${project_version}-${target_platform}"

if [[ "${OFFLINE}" -eq 1 ]]; then
    release_name="mq-auth-server-wheel-offline-${project_version}-${target_platform}"
fi
...
validate_required_files "${PROJECT_DIR}" \
    ...
    scripts/systemd/mq-auth-server.service \
    app/acs
...
wheel_path="$(find "${OUTPUT_DIR}" -maxdepth 1 -type f -name "mq_auth_server-${project_version}-*.whl" | LC_ALL=C sort | tail -n 1)"
if [[ -z "${wheel_path}" ]]; then
    echo "错误：未在 dist/ 下找到 mq_auth_server-${project_version}-*.whl" >&2
    exit 1
fi
...
RUNTIME_BUNDLE_MAP=(
    ...
    "scripts/systemd/mq-auth-server.service|mq-auth-server.service"
    "app/acs|acs"
)
