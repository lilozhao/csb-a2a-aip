...
release_name="demo-leader-wheel-${project_version}-${target_platform}"

if [[ "${OFFLINE}" -eq 1 ]]; then
    release_name="demo-leader-wheel-offline-${project_version}-${target_platform}"
fi
...
validate_required_files "${PROJECT_DIR}" \
    ...
    scripts/systemd/demo-leader-api.service \
    scripts/systemd/demo-leader-web.service
...
app_wheel_path="$(find_wheel "${OUTPUT_DIR}" "demo_leader-${project_version}-*.whl")"
...
if [[ -z "${app_wheel_path}" ]]; then
    echo "错误：未在 dist/ 下找到 demo_leader-${project_version}-*.whl" >&2
    exit 1
fi
...
RUNTIME_BUNDLE_MAP=(
    ...
    "README.md|README.md"
    "leader/config.toml|leader/config.toml"
    "leader/atr|leader/atr"
    "leader/scenario|leader/scenario"
    "web_app|web_app"
    ...
    "scripts/smoke/business.py|scripts/smoke/business.py"
    "scripts/start-leader-api.sh|scripts/start-leader-api.sh"
    "scripts/start-web-ui.sh|scripts/start-web-ui.sh"
    "scripts/systemd/demo-leader-api.service|demo-leader-api.service"
    "scripts/systemd/demo-leader-web.service|demo-leader-web.service"
)
...
chmod +x \
    "${staging_dir}/scripts/start-leader-api.sh" \
    "${staging_dir}/scripts/start-web-ui.sh" \
    "${staging_dir}/scripts/smoke-test.sh" \
    "${staging_dir}/scripts/smoke-test-business.sh"
