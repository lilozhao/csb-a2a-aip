...
release_name="registry-server-wheel-${project_version}-${target_platform}"

if [[ "${OFFLINE}" -eq 1 ]]; then
    release_name="registry-server-wheel-offline-${project_version}-${target_platform}"
fi
...
wheel_path="$(find "${OUTPUT_DIR}" -maxdepth 1 -type f -name "registry_server-${project_version}-*.whl" | LC_ALL=C sort | tail -n 1)"
if [[ -z "${wheel_path}" ]]; then
    echo "错误：未在 dist/ 下找到 registry_server-${project_version}-*.whl" >&2
    exit 1
fi
...
RUNTIME_BUNDLE_MAP=(
    ...
    "README.md|README.md"
    "scripts/systemd/registry-server.service|registry-server.service"
)
