#!/usr/bin/env bash
# 自动部署脚本（deploy.sh，支持首次部署与版本更新）
# 用法: ./deploy.sh [--rollback]
#
# 功能：自动检测首次部署或版本更新，执行对应操作
#   首次部署：启动基础设施 → 执行迁移 → 启动 registry-server-blue
#   版本更新：加载镜像 → 执行迁移 → 蓝绿切换
#   回滚：    将流量切回上一个颜色
#
# 选项：
...
HEALTH_TIMEOUT=60
HEALTH_INTERVAL=3
APP_CONTAINER_PREFIX="registry-server"
export COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-registry-release-bundle}"
...
upgrade_infra() {
    if [[ "$NEED_POSTGRES" == "true" ]]; then
        ...
        # 注：容器停止后 compose up -d 会自动重建
    fi

    if [[ "$NEED_POSTGRES" == "true" ]]; then
        log "启动新版本基础设施..."
        compose_up_detached -f "$COMPOSE_FILE" up -d postgres
        # 等待数据库就绪（postgres 升级时需要）
        log "等待数据库就绪..."
        for i in {1..30}; do
            if docker compose -f "$COMPOSE_FILE" exec -T postgres pg_isready -U "${POSTGRES_INIT_USER:-registry}" &>/dev/null; then
                log "数据库就绪"
                return 0
            fi
            sleep 2
...
# 检测是否首次部署（以应用容器是否存在为准，允许从半截首发状态恢复）
is_first_deploy() {
...
# 启动应用（首次部署模式）
start_app_blue() {
    # 首发时先写 upstream，再启动 nginx，避免启动后立即 reload 带来的瞬时 502。
    write_upstream "blue"
...
get_active_color() {
    ...
    if grep -q "registry-server-green:9001" "$upstream_conf" 2>/dev/null; then
        echo "green"
    else
        echo "blue"
    fi
...
map \$request_uri \$app_backend {
    default http://$(get_color_container_name "$color"):9001;
}
EOF
...
run_smoke_test() {
    ...
    local base_url="http://localhost:${NGINX_PORT:-9001}"
...
main() {
    ...
    if is_first_deploy; then
        ...
        log ""
        log "=== 首次部署完成 ==="
        log "应用地址：http://localhost:${NGINX_PORT:-9001}"
        log ""
        log "提示："
