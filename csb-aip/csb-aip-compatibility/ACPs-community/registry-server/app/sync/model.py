...
class ChangeLog(SQLModel, table=True):
    ...
    __tablename__ = "change_log"  # pyright: ignore[reportAssignmentType, reportIncompatibleVariableOverride]
    ...
    # 时间戳
    ts: datetime = Field(default_factory=get_beijing_time, sa_column=Column(TIMESTAMP(timezone=True)))
...
class Snapshot(SQLModel, table=True):
    ...
    from_seq: int | None = Field(default=None, sa_column=Column(BigInteger))

    # 快照是否被删除
    is_deleted: bool = Field(default=False, index=True)

    # 创建时间
    created_at: datetime = Field(default_factory=get_beijing_time, sa_column=Column(TIMESTAMP(timezone=True)))

    # 最后访问时间
    last_access_at: datetime = Field(default_factory=get_beijing_time, sa_column=Column(TIMESTAMP(timezone=True)))
...
class WebHook(TimestampMixin, SQLModel, table=True):
    ...
    # 下次重试时间
    next_retry_at: datetime | None = Field(default=None, sa_column=Column(TIMESTAMP(timezone=True)))

    # 最后触发时间
    last_triggered_at: datetime | None = Field(default=None, sa_column=Column(TIMESTAMP(timezone=True)))

    # 最后成功时间
    last_success_at: datetime | None = Field(default=None, sa_column=Column(TIMESTAMP(timezone=True)))

    # 最后失败时间
    last_failure_at: datetime | None = Field(default=None, sa_column=Column(TIMESTAMP(timezone=True)))

    # 最后失败原因
    last_failure_reason: str | None = Field(default=None, sa_column=Column(Text))
