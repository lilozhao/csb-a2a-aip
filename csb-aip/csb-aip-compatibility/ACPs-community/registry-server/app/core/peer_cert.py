...
def _extract_common_name(peer_certificate: dict[str, Any] | None) -> str | None:
...
class PeerCertificateRegistry:
    ...
    def register(
        ...
        """Register a peer certificate for the active connection."""
        key = (client[0], client[1], server[0], server[1])
        info = PeerCertificateInfo(
            common_name=_extract_common_name(certificate),
    ...
    def unregister(self, client: tuple[str, int], server: tuple[str, int]) -> None:
        """Drop the stored peer certificate for a closed connection."""
        key = (client[0], client[1], server[0], server[1])
        with self._lock:
    ...
    def lookup(
        ...
        """Lookup the peer certificate for the active connection."""
        key = (client[0], client[1], server[0], server[1])
        with self._lock:
