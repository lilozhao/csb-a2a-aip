...
down_revision: str | None = "841699557267"
...
def upgrade() -> None:
    ...
    op.add_column("agent", sa.Column("deleted_at", sa.TIMESTAMP(timezone=True), nullable=True))
    op.add_column(
        "agent",
        sa.Column(
            "deleted_reason",
    ...
    op.add_column("agent", sa.Column("disabled_at", sa.TIMESTAMP(timezone=True), nullable=True))
    op.add_column(
        "agent",
        sa.Column(
            "disabled_reason",
    ...
    op.create_index(op.f("ix_agent_acs_last_seq"), "agent", ["acs_last_seq"], unique=False)
