#!/usr/bin/env bash
# build-release-bundle.sh — 构建镜像并打包为统一可部署产物
# 用法: ./scripts/release-bundle/build-release-bundle.sh [--dry-run] [VERSION]
#
# 输出: dist/ca-server-release-{VERSION}.tar.gz（包含镜像 + 配置 + 部署脚本 + 文档）
#
# 选项：
...
REQUIRED_FILES=(
    ...
    "scripts/release-bundle/nginx/conf.d/ca-server.conf"
...
BUNDLE_MAP=(
    ...
    "scripts/release-bundle/nginx/conf.d/ca-server.conf|nginx/conf.d/ca-server.conf"
...
RELEASE_NAME="ca-server-release-${VERSION}"
