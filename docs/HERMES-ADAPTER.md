...
### 5.3 ⚠️ 症状识别法（第二轮之后立，结论已于第三轮更正）

**症状识别法（保留，仍然有效）**：
- **无 `parsed=` 行、且 `path=` 显示默认值** = **根本没读**（首轮：path 未透传）
- **有轮询、`path=` 正确、却无 `parsed=`** = 在读了，但 **① 返回结构/文本没进来**（第三轮真因 #2）或 **② 库里还没那条**
  → 先查 `collectTexts` 那一层，再查时序

**❌ 已更正（2026-09-16 第三轮）**：曾一度把第二轮的失败判为「**宿主交互时序**（长回合阻塞落库）」，
**该判断已被实测推翻并撤回** —— 主人的确认在窗口起点 **+21s 就落库了**，bridge 之后又轮询了几十次。
真因是 **K17 返回结构错位**（`collectTexts(resp.result)` 拿到 undefined）。

**仍建议保留的两条工程习惯**（与上述因果无关，属防御性设计）：
1. **委托内容不宜要求「等确认后的证据」** —— 让注入回合尽快结束、回执下一轮给，可避免把一次委托拖成长任务
2. **接收方确认窗口可适当调大**（重回合宿主建议 10min）
