...
### 5.3 ⚠️ 宿主交互时序约束（2026-09-16 墨丘实测·第二轮）

**现象**：`path=db` 已生效（读回腿**确实在读**），60 条轮询却始终无 `parsed=` 行——第二轮仍 `confirm_timeout`。
**根因（宿主侧，非 bridge）**：**消息持久化发生在回合边界**。当注入回合自己跑一个长循环时，主人的「确认 #…」还没落库；而读回腿查的正是 `state.db`。
⇒ 等于在等一个**只有回合结束才会出现的条件**。

**两条铁律**（写进委托设计约定）：
1. **委托内容不得要求「等确认后的证据」** —— 否则注入回合自阻塞，形成循环依赖。
   正确做法：注入回合**尽快结束**（只做那件写操作）；确认过程与回执由**下一轮 / 异步**给。
2. **接收方需要把确认窗口调大** —— 宿主回合重时 5 分钟不够；建议 `A2A_BRIDGE_CONFIRM_TIMEOUT_MS=600000`（10min）。

**症状识别法（背下来）**：
- **无 `parsed=` 行** = 根本没读（如首轮 `path` 未透传）
- **有轮询、`path=db` 正确、但无 `parsed=`** = 在读了，**库里还没那条消息**（时序问题）
