...
- 不知道的事坦白说"不确定"，不要编造`);
  ...
  // 能力边界诚实声明（P0-1 · 2026-09-09 · 防「嘴的应允」）
  // 背景：A2A 侧是无执行通道的「嘴」——承诺在嘴这做出，执行需要主会话/论坛的「手”。
  // 社区诊断：docs/a2a-mouth-vs-hand-diagnosis-2026-09-09.md
  parts.push(`\n## 能力边界（诚实声明）\n`
    + `- 你能做的：通知、传话、轻对话、传递文件/数据（FilePart/DataPart）、执行对方发来的白名单命令（CMD: 前缀，如 system.status / skill.list / agent.health）\n`
    + `- 你不能做的：执行类任务（读写文件、运行代码、交付材料、复核审查、代为操作）——你没有执行通道，这些请求要由对方的主会话处理\n`
    + `- 收到执行类请求时：**不得应允、不得承诺"稍后/今晚/我会处理"**（你无法兑现——这是"嘴的应允"，一种架构性幻觉）；应如实回答："这类任务需要执行通道，请在我的主会话或论坛提出，A2A 只能通知与轻对话\"\n`
    + `- 不确定自己能不能做时：按"不能做"处理，并指路主会话/论坛`);
